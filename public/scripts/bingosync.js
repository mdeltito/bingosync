...
;(function() {
  ...
  socket.on('update userlist', function(users) {
    ...
    _.each(users, function(user) {
      return user.points = $('#board').find("." + user.color).length;
    });
    $('#user-list').html(templates.user_list({
      users: users
    }));
    return bingo.board.trigger('update');
  });
  ...
  socket.on('chat message', function(client, message, timestamp) {
    var item;
  ...
  $(function() {
    ...
    $('#join-session').submit((function(_this) {
      return function(e) {
        ...
        _this.bingo.seed = $('#seed').val();
        _this.bingo.type = $('#type').val();
        _this.bingo.nickname = $('#nickname').val();
        _this.bingo.name = $('#session_name').val();
        _this.bingo.password = $('#password').val();
        _this.bingo.color = user_color();
        if (!_.every(_this.bingo)) {
    ...
    $('#chat-message').on('focus', function(e) {
      return $(this).closest('.panel').removeClass('panel-warning');
    });
    return bingo.board.bind('update', function(data) {
      _.each(bingo.users, function(user) {
        var count;
        count = bingo.board.find(".btn-" + user.color).length;
        return $(".badge-" + user.color).text(count);
      });
      return apply_planning();
    });
  });
  ...
  templates = {
    ...
    chat_message: _.template('' + '<li class="clearfix"> <div class="chat-body clearfix"> <div class="header"> <strong class="primary-font"><%= user.nickname %></strong> <small class="pull-right text-muted"><span class="glyphicon glyphicon-time"></span><%= time %></small> </div> <p><%= message %></p> </div> </li>')
  };
  ...
  apply_planning = function() {
    $('.glyphicon-bookmark', '#board').remove();
    return _.forEach(planning_ids, function(id) {
      return $("#" + id).append('<span class="glyphicon glyphicon-bookmark"></span>');
    });
  };
  ...
  chat_requires_highlight = function() {
    return $('#chat-widget .panel-body').hasClass('collapsed') || (!$('#chat-send').is(':focus, :active') && !$('#chat-message').is(':focus'));
  };
